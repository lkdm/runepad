...
function applyListStyle(textarea, style) {
  const result = applyLineOperation(textarea, (ta) => listStyle(ta, style), {
...
function toggleQuote(textarea) {
  ...
  const result = applyLineOperation(textarea, (ta) => multilineStyle(ta, style), { prefix: style.prefix });
  debugResult(result);
  insertText(textarea, result);
...
`) {
  ...
  const result = applyLineOperation(textarea, (ta) => {
...
var _OverType = class _OverType2 {
  constructor(target, options = {}) {
    ...
    const instances = elements.map((element) => {
      ...
      const instance = Object.create(_OverType2.prototype);
      instance._init(element, options);
      element.overTypeInstance = instance;
      _OverType2.instances.set(element, instance);
      return instance;
    });
    return instances;
  }
  _init(element, options = {}) {
    ...
    this.instanceId = ++_OverType2.instanceCount;
    this.initialized = false;
    _OverType2.injectStyles();
    _OverType2.initGlobalListeners();
  ...
  _recoverFromDOM(container, wrapper) {
    if (container && container.classList.contains("overtype-container")) {
      ...
      const themeToUse = this.instanceTheme || _OverType2.currentTheme || solar;
...
`);
  ...
  destroy() {
    ...
    _OverType2.instances.delete(this.element);
    if (this.shortcuts) {
  ...
  static init(target, options = {}) {
    return new _OverType2(target, options);
  }
  static getInstance(element) {
    return element.overTypeInstance || _OverType2.instances.get(element) || null;
  }
  static destroyAll() {
    const elements = document.querySelectorAll("[data-overtype-instance]");
    elements.forEach((element) => {
      const instance = _OverType2.getInstance(element);
      if (instance) {
        instance.destroy();
      }
    });
  ...
  static setTheme(theme, customColors = null) {
    ...
    _OverType2.currentTheme = themeObj;
    _OverType2.injectStyles(true);
  ...
  static initGlobalListeners() {
    if (_OverType2.globalListenersInitialized)
      return;
    document.addEventListener("input", (e) => {
      if (e.target && e.target.classList && e.target.classList.contains("overtype-input")) {
        ...
        const instance = wrapper == null ? undefined : wrapper._instance;
        if (instance)
    ...
    _OverType2.globalListenersInitialized = true;
  }
};
...
var [editor] = new overtype_default("#editor", {
  autofocus: true,
  minHeight: "90vh",
  autoResize: true,
  textareaProps: {
...
function openHandler() {
  const loadButton2 = document.getElementById("loadButton");
  if (loadButton2) {
    loadButton2.click();
  }
}
